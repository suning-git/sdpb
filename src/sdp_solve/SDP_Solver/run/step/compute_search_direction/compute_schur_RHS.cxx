...
void El_DistMatrix_symmetrize(El::DistMatrix<El::BigFloat> & block)
{
                       ...
                       Block_Vector &dx)
{
  ...
  std::vector<El::DistMatrix<El::BigFloat>> bilinear_bases_copy(sdp.bilinear_bases);

  auto bilinear_bases_block(bilinear_bases_copy.begin());
            ...
            for(size_t row_block = 0; row_block <= column_block; ++row_block)
              {
				...
				//El_DistMatrix_symmetrize(*bilinear_bases_block);
...
void compute_schur_RHS_org(const Block_Info &block_info, const SDP &sdp,
	const Block_Vector &dual_residues,
	const Block_Diagonal_Matrix &Z,
	Block_Vector &dx)
{
